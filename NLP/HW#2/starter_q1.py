...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        seq_len = x.size(1)
        pe_slice = Variable(self.pe[:,:seq_len], requires_grad=False)
...
def get_model(opt, vocab_size):
    ...
    if opt.loadname is not None:
        ...
        try:
            ...
            print(f"Error: Pretrained weights file not found at {opt.loadname}. Initializing weights randomly.")
            for p in model.parameters():
                if p.dim() > 1: nn.init.xavier_uniform_(p)
        except Exception as e:
             print(f"Error loading weights: {e}. Initializing weights randomly.")
             for p in model.parameters():
                if p.dim() > 1: nn.init.xavier_uniform_(p)
    else:
