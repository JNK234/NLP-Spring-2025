...
train_data = load_jsonl("/home/kqm0007/northwestern/NLP-Spring-2025/homework/data/train_complete.jsonl")
val_data = load_jsonl("/home/kqm0007/northwestern/NLP-Spring-2025/homework/data/dev_complete.jsonl")
...
if args.mode == "adapter":
    from model_adapter import BertAdapter
    model = BertAdapter().to(device)
elif args.mode == "lora":
    from model_lora import BertLoRA
    model = BertLoRA().to(device)
elif args.mode == "prefix":
    from model_prefix import BertPrefix
    model = BertPrefix().to(device)
else:
    from model import BertInitial
    model = BertInitial(dropout=args.dropout).to(device)
...
for epoch in range(args.epochs):
    ...
    logging.info(f"[Epoch {epoch+1}/{args.epochs}] Train Loss: {avg_loss:.4f} | Train Accuracy: {accuracy:.2f}% | Time: {end_time - start_time:.2f}s")
...
start = time.time()
test_data = load_jsonl("/home/kqm0007/northwestern/NLP-Spring-2025/homework/data/train_complete.jsonl")
